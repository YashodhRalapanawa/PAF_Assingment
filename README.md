...
This repository contains a full-stack campus management application with a React/Vite frontend and a Spring Boot backend.

## Project structure

- `frontend/` — React app built with Vite.
- `backend/` — Spring Boot 3.3.2 application targeting Java 21.
- `package.json` — root package file with shared dependency metadata.

## Tech stack

- Frontend: React 18.3, Vite, React Router, Axios, Recharts, SweetAlert2, Framer Motion.
- Backend: Spring Boot, Spring Data JPA, Spring Security, OAuth2 client, validation, OpenPDF, MySQL connector.
...
3. API configuration:
   - `frontend/src/api/apiClient.js` uses `import.meta.env.VITE_API_BASE_URL` or defaults to `http://localhost:8085/api`.
   - The app uses session cookies (`withCredentials: true`).

## Resource feature (frontend)

This document describes the **campus resources** UI: listing, filtering, charts, PDF export, and admin-only create/update/delete. It is intentionally **not committed** to the remote repository unless you add this file explicitly to `.gitignore`.
...
| File | Purpose |
|------|---------|
| `frontend/src/components/resource/resource.css` | Layout shell, cards, nav-related resource page styling. |
| `frontend/src/components/resource/table.css` | Table, pills, buttons, modals used by the list and form. |
...
- `backend/target/`, `backend/uploads/`, and `.env` files are ignored by `.gitignore`.
- If you want this file to remain local-only, add `README.md` to `.gitignore`.
